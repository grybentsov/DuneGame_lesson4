...
public class Tank extends GameObject implements Poolable {
    ...
    private TextureRegion containerbarTexture;
    private int hp;
    private float angle;
    private float speed;
    ...
    public Tank(GameController gc) {
        ...
        this.containerbarTexture = Assets.getInstance().getAtlas().findRegion("containerbar");
    ...
    public boolean choiceActiveTank(){
        if (container > 50){
            return ownerType == Owner.DISABLED;
        }
        return ownerType == Owner.PLAYER;
    }

    public void updateWeapon(float dt) {
        if (weapon.getType() == Weapon.Type.HARVEST) {              // если мы harvester
            if (gc.getMap().getResourceCount(this) > 0) {   // находимся в клетке, где больше 0 ресурса
                int result = weapon.use(dt);
                if (result > -1 && container <= 50) {
                    container += gc.getMap().harvestResource(this, result); // танк собирает ресурс с указанной силой и + к контейнеру
                    } else {
                    choiceActiveTank();
                }
                } else {
                       weapon.reset();         // сброс weapon
                }
            }
        }
    ...
    public void render(SpriteBatch batch) {
        ...
        if (weapon.getType() == Weapon.Type.HARVEST && weapon.getUsageTimePercentage() > 0.0f) {
            ...
            batch.draw(containerbarTexture, position.x - 32, position.y + 60, 64, 12);
            batch.setColor(1.0f, 1.0f, 0.0f, 1.0f);
            batch.draw(progressbarTexture, position.x - 30, position.y + 32, 60 * weapon.getUsageTimePercentage(), 8);
            batch.setColor(1.0f, 1.0f, 1.0f, 1.0f);
            batch.draw(containerbarTexture, position.x - 30, position.y + 62, container, 8);
        }
    }
}